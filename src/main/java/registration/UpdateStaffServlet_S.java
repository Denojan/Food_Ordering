...
/**
 * Denojan
 * Servlet implementation class updateStaffServlet_S
 */
@WebServlet("/updatestaffservlet_S")
public class UpdateStaffServlet_S extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(name == null || name.equals("")) {
			...
			dispatcher = request.getRequestDispatcher("updateStaffForm_S.jsp");
...
try{
	...
	Person person = new Staff(name,password,mobile,address);
	person.setemail((String)session.getAttribute("email"));

	
	PreparedStatement pst1 = con.prepareStatement("select password from staff where email=?");
	...
	person.setpassword(encrpytedPassword);
		...
		if(encrpytedPassword.equals(Password)) {
			...
			PreparedStatement pst = con.prepareStatement("update staff set name = ? ,mobile=?, address=? where email =?");
			...
			if(rowCount != 0) {
				...
				response.sendRedirect("StaffDetail_S.jsp");
